(function() {
  ...
  function create(deps) {
    ...
    function applyWinnerGroupAliases(params) {
      ...
      const mainName = normalizeDisplay(input.nextMainName);
      const targetKey = toMatchKey(mainName);
      if (!targetKey) {
        return null;
      }
      ...
      const addCandidate = (value) => {
        const aliasDisplay = normalizeDisplay(value);
        const aliasKey = toMatchKey(aliasDisplay);
        if (!aliasKey) {
          return;
        }
    ...
    function removeWinnerGroupData(params) {
      const input = params && typeof params === 'object' ? params : {};
      const entry = input.entry;
      ...
      const canonicalKey = normalizeText(entry.canonicalKey);
      const keySet = new Set(
        [entryKey, canonicalKey, ...rawKeys.map((rawKey) => normalizeText(rawKey))]
          .filter(Boolean)
      );
      if (!keySet.size) {
        return null;
      ...
      const aliasEntries = parseAliasRuleEntries(input.aliasRulesText);
      Array.from(aliasEntries.keys()).forEach((aliasKey) => {
        const normalizedAliasKey = normalizeText(aliasKey);
        if (!normalizedAliasKey) {
          return;
        }

        const aliasEntry = aliasEntries.get(aliasKey);
        if (!aliasEntry) {
          return;
        }

        if (keySet.has(normalizedAliasKey)) {
          aliasEntries.delete(aliasKey);
          return;
        }

        keySet.forEach((key) => {
          aliasEntry.aliases.delete(key);
        });
      });
